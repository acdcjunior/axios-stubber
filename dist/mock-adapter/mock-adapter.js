    ...
    adapter() {
        return function (config) {
            ...
            if (arguments.length === 3) {
                (0, handle_request_1.default)(mockAdapter, arguments[0], arguments[1], arguments[2]);
            }
            else {
                return new Promise(function (resolve, reject) {
                    (0, handle_request_1.default)(mockAdapter, resolve, reject, config);
                });
            }
        }.bind(this);
    }
        ...
        return {
            ...
            abortRequest: function () {
                return reply(function (config) {
                    const error = utils.createAxiosError("Request aborted", config, undefined, "ECONNABORTED");
            ...
            timeout: function () {
                return reply(function (config) {
                    const error = utils.createAxiosError(config.timeoutErrorMessage ||
                        "timeout of " + config.timeout + "ms exceeded", config, undefined, "ECONNABORTED");
                    return Promise.reject(error);
                });
            },
...
function findInHandlers(method, handlers, handler) {
    ...
    for (let i = 0; i < handlers[method].length; i += 1) {
        ...
        const comparePaths = item[0] instanceof RegExp && handler[0] instanceof RegExp
            ? String(item[0]) === String(handler[0])
            : item[0] === handler[0];
        const isSame = comparePaths &&
